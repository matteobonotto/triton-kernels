...
def _eager_linear_fwd(x: Tensor, W: Tensor, b: Optional[Tensor] = None) -> Tensor:
    if b is not None:
        return x @ W.T + b
    return x @ W.T


def _eager_linear_bwd(x, W, b, grad_output):
...
class LinearFunction(Function):
    ...
    def forward(ctx, x: Tensor, W: Tensor, b: Tensor):
        ...
        return _eager_linear_fwd(x, W, b)
    ...
    def backward(
        ...
        grad_x, grad_W, grad_b = _eager_linear_bwd(x, W, b, grad_output)
        return grad_x, grad_W, grad_b
